...
namespace Gravityzero.Console.Utility.Commands
{
    public class DeleteAnimalCommand : BaseCommand<AnimalArguments>
    {
        ...
        protected override CommandResult Execute(ConsoleContext context, AnimalArguments arguments)
        {
            ...
            if(existAnimal.Result.Response.Payload.Count()==1){
                var result = WinApiConnector.RequestDelete<Response<string>>($"{context.ConsoleSettings.ServerAddress}:{context.ConsoleSettings.Port}/Api"+$"/{existAnimal.Result.Response.Payload.FirstOrDefault().Identifier}");
                return new CommandResult(result.Result.IsSuccess ? "OK" : result.Result.Message);
            }
            else if(existAnimal.Result.Response.Payload.Count()>1){
                System.Console.WriteLine($"Znaleziono więcej niż jeden obiekt o nazwie {arguments.Name} ({existAnimal.Result.Response.Payload.Count()})");
                System.Console.WriteLine($"Dokonaj updatu jednej z nazw ({arguments.Name})");
                return new CommandResult();
            }
            else if(existAnimal.Result.Response.Payload.Count()==0){
